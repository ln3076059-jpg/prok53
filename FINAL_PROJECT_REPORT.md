...
is implemented, but independently reviewed association annotations are required. The existing event-truth freeze artifact is historical and is not the final post-calibration event holdout. Final system-level event evaluation requires a newly frozen, untouched sequence holdout with independent event annotations. Event
