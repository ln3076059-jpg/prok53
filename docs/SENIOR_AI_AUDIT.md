...
| HIGH | Review delegation could be mistaken for human approval | AI/admin-delegated work needs a distinct audit identity and append-only confirmation boundary | Governed provenance could be overstated | `training/run_review1_review.py`, `tools/annotation_reviewer/app.py` | Store `review1` + `reviewer_type: AI`, use `REVIEW1_*`, and require a separate explicit admin action for HUMAN records | IMPLEMENTED; 100 VISUALLY REVIEWED; 0 HUMAN CONFIRMED |
...
| HIGH | Concurrent demo jobs could overlap process-wide inference resources | Separate requests could dispatch `BackgroundTasks` concurrently while the detector and GPU are process-wide | Tracker/model/cache collisions or nondeterministic GPU failures | `backend/services/analysis_worker.py` | Serialize the complete single-process demo inference worker and test two concurrent jobs | IMPLEMENTED |
| HIGH | Model lock did not bind calibration to the locked weights and runtime thresholds | Calibration provenance was recorded but not cross-checked during lock creation | A model-A threshold artifact could be attached to model B | `training/lock_model.py`, `training/calibrate_thresholds.py` | Verify calibration model SHA, exact classes/thresholds, all configured component hashes and immutable outputs | IMPLEMENTED_TOOLING; NOT_RUN |
| HIGH | Configured auxiliary/windshield artifacts were absent from production lock comparison | Runtime checked specialist detector hashes only | Classifier, pose, vehicle or windshield weights could change outside the lock | `backend/ai/detector.py`, `training/lock_model.py` | Hash every configured component and recompute the complete map at production startup | IMPLEMENTED |
| HIGH | Local reviewer implicitly assigned `reviewer_type=HUMAN` | The field had a server-side default and retry requests appended duplicate decisions | Software could manufacture human provenance by omission | `tools/annotation_reviewer/app.py` | Require explicit HUMAN assertion, serialize append operations and make identical retries idempotent | IMPLEMENTED_TOOLING |
| MEDIUM | Evidence root containment was incomplete | Event identifiers were joined to evidence paths before a base-root containment check | A malformed persisted identifier could escape the evidence directory | `backend/ai/evidence.py`, `backend/api/routes.py` | Resolve and reject every path outside the configured evidence root | IMPLEMENTED |
