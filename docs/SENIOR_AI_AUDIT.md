...
Repository state reviewed: `main` at `ba1f0a4`
...
| MEDIUM | Temporal logic is observation-count-centric | No hysteresis state, gap tolerance, or structured feature trace | Flicker, track gaps and opaque decisions | `backend/ai/events.py`, `backend/ai/sequence.py` | Add EMA/ratio/duration/gap features, activation/release and reset behavior | IMPLEMENTED |
...
| MEDIUM | Evidence is a single moment | Only original and annotated JPEGs are stored | Multi-frame behavior decisions cannot be audited | `backend/services/video_analyzer.py`, `backend/models/entities.py` | Add buffered pre/post clip and JSON trace with SHA-256 | IMPLEMENTED |
