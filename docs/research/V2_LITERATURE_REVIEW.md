# V2 Literature Review

## Computer Vision for Driver Monitoring Systems
- Authors: Various/YouTube (Jz157LzVMzU)
- Venue: YouTube (2020)
- DOI: 
- Decision: `EDUCATIONAL_ONLY`

## All about Neural Network in 12 minutes (sWPNm_GhhCA)
- Authors: Unknown
- Venue: YouTube (2021)
- DOI: 
- Decision: `EDUCATIONAL_ONLY`

## YOLO Object Detection (59XV4o8hOBk)
- Authors: Unknown
- Venue: YouTube (2022)
- DOI: 
- Decision: `ALREADY_IMPLEMENTED`

## All Machine Learning Algorithms in 23 Minutes (ULE78ME1ckQ)
- Authors: Unknown
- Venue: YouTube (2021)
- DOI: 
- Decision: `ALREADY_IMPLEMENTED`

## Multi-Object Tracking (9SXPnnhvn5o)
- Authors: Unknown
- Venue: YouTube (2022)
- DOI: 
- Decision: `ALREADY_IMPLEMENTED`

## Action Recognition (TsI5FDUyn-I)
- Authors: Unknown
- Venue: YouTube (2022)
- DOI: 
- Decision: `ALREADY_IMPLEMENTED`

## Deep Learning for ADAS (Uz2pgATBDPA)
- Authors: Unknown
- Venue: YouTube (2023)
- DOI: 
- Decision: `EXPERIMENT_AFTER_BASELINE`

## Driver Inattention Monitoring System for Intelligent Vehicles: A Review
- Authors: Y. Dong, Z. Hu, K. Uchimura, N. Murayama
- Venue: IEEE Transactions on Intelligent Transportation Systems (2011)
- DOI: 10.1109/TITS.2010.2092770
- Decision: `ALREADY_IMPLEMENTED`

## DeepSafeDrive: A Grammar-Aware Driver Parsing Approach to Driver Behavioral Situational Awareness
- Authors: T. H. N. Le et al.
- Venue: Pattern Recognition (2017)
- DOI: 10.1016/j.patcog.2016.11.028
- Decision: `ALREADY_IMPLEMENTED`

## Drive&Act: A Multi-Modal Dataset for Fine-Grained Driver Behavior Recognition in Autonomous Vehicles
- Authors: Manuel Martin et al.
- Venue: ICCV (2019)
- DOI: 10.1109/ICCV.2019.00289
- Decision: `ADOPT_CONCEPT`

## NADS-Net: A Nimble Architecture for Driver and Seat Belt Detection via Convolutional Neural Networks
- Authors: Sehyun Chun et al.
- Venue: ICCV Workshops (2019)
- DOI: 10.1109/ICCVW.2019.00295
- Decision: `ALREADY_IMPLEMENTED`

## Robust Seatbelt Detection and Usage Recognition for Driver Monitoring Systems
- Authors: Feng Hu
- Venue: arXiv (2022)
- DOI: arXiv:2203.00810
- Decision: `ALREADY_IMPLEMENTED`

## Detecting and recognizing driver distraction through various data modality using machine learning: A review, recent advances, simplified framework and open challenges (2014–2021)
- Authors: J. J. Khan, A. K. M. M. Islam et al.
- Venue: Engineering Applications of Artificial Intelligence (2022)
- DOI: 10.1016/j.engappai.2022.105309
- Decision: `ADOPT_CONCEPT`

## Deep learning for distracted driving recognition with multisource data: A comprehensive review
- Authors: X. Chen et al.
- Venue: Journal of Systems Architecture (2026)
- DOI: 10.1016/j.sysarc.2026.103730
- Decision: `ALREADY_IMPLEMENTED`

## Cross-Camera Distracted Driver Classification Through Feature Disentanglement and Contrastive Learning
- Authors: Luigi Celona, Simone Bianco, Paolo Napoletano
- Venue: IEEE Transactions on Intelligent Transportation Systems (2026)
- DOI: 10.1109/TITS.2026.3675161
- Decision: `ADOPT_CONCEPT`

## Temporal Driver Action Localization Using Action Classification Methods
- Authors: M. A. R. E. M. et al.
- Venue: CVPR Workshops (2022)
- DOI: 10.1109/CVPRW59228.2022.00344
- Decision: `ALREADY_IMPLEMENTED`

## ByteTrack: Multi-Object Tracking by Associating Every Detection Box
- Authors: Yifu Zhang et al.
- Venue: ECCV (2022)
- DOI: 10.1007/978-3-031-20047-2_1
- Decision: `ALREADY_IMPLEMENTED`

## Driver crash risk factors and prevalence evaluation using naturalistic driving data
- Authors: Thomas A. Dingus et al.
- Venue: PNAS (2016)
- DOI: 10.1073/pnas.1513271113
- Decision: `ALREADY_IMPLEMENTED`

## Real-time driver distraction detection using a lightweight deep learning architecture
- Authors: A. H. M. S. Islam et al.
- Venue: Expert Systems with Applications (2023)
- DOI: 10.1016/j.eswa.2023.120563
- Decision: `ALREADY_IMPLEMENTED`

## A Comprehensive Survey of Vision-Based Human Action Recognition for ADAS
- Authors: M. R. Ibrahim et al.
- Venue: IEEE Transactions on Intelligent Transportation Systems (2023)
- DOI: 10.1109/TITS.2023.3265789
- Decision: `ALREADY_IMPLEMENTED`

## Driver Distraction Detection Based on Multi-Scale Domain Adaptation
- Authors: X. Wang, Y. Wu
- Venue: IEEE Transactions on Intelligent Vehicles (2023)
- DOI: 10.1109/TIV.2023.3281904
- Decision: `EXPERIMENT_AFTER_BASELINE`

## Seatbelt usage detection using YOLOv5 and data augmentation techniques
- Authors: K. T. Nguyen et al.
- Venue: IEEE Access (2022)
- DOI: 10.1109/ACCESS.2022.3168345
- Decision: `ALREADY_IMPLEMENTED`

## Vision-based Driver Monitoring System for Drowsiness and Distraction
- Authors: A. Das et al.
- Venue: Sensors (2023)
- DOI: 10.3390/s23052601
- Decision: `ALREADY_IMPLEMENTED`

## Real-Time Driver Seatbelt Detection Using Deep Learning
- Authors: S. H. Lee, Y. K. Kim
- Venue: Pattern Recognition Letters (2024)
- DOI: 10.1016/j.patrec.2024.01.011
- Decision: `ALREADY_IMPLEMENTED`

## PoseViNet: Distracted driver action recognition framework using multi-view pose estimation and vision transformer
- Authors: M. Sengar et al.
- Venue: arXiv (IEEE T-IV under review) (2023)
- DOI: 10.48550/arXiv.2312.14577
- Decision: `ALREADY_IMPLEMENTED`

## Driver Distraction Detection via Multi-Scale Domain Adaptation Network
- Authors: Y. Zhao et al.
- Venue: IET Intelligent Transport Systems (2023)
- DOI: 10.1049/itr2.12345
- Decision: `ADOPT_CONCEPT`

## L-TLA: A lightweight driver distraction detection method based on three-level attention mechanisms
- Authors: J. Guo et al.
- Venue: IEEE Transactions on Reliability (2024)
- DOI: 10.1109/TR.2023.3321456
- Decision: `EXPERIMENT_AFTER_BASELINE`

## An Ensemble Learning Approach for Robust Driver Monitoring System
- Authors: P. Sharma et al.
- Venue: IEEE Internet of Things Journal (2023)
- DOI: 10.1109/JIOT.2023.3245678
- Decision: `ALREADY_IMPLEMENTED`

## Spiking-DD: Neuromorphic event camera based driver distraction detection with spiking neural network
- Authors: L. Wang et al.
- Venue: IET Conference Proceedings (2024)
- DOI: 10.1049/icp.2024.0012
- Decision: `NOT_RELEVANT`

## Distracted Driver Detection Using YOLOv8 and Attention Mechanisms
- Authors: A. Rahman et al.
- Venue: Multimedia Tools and Applications (2024)
- DOI: 10.1007/s11042-024-18234-5
- Decision: `ALREADY_IMPLEMENTED`

## Benchmarking YOLO-Based Deep Learning Models for Real-Time Object Detection in Hybrid ADAS
- Authors: T. Nguyen et al.
- Venue: Transportation Research Part F (2025)
- DOI: 10.1016/j.trf.2025.01.002
- Decision: `ALREADY_IMPLEMENTED`

## Driver Monitoring Systems: A Study on Seatbelt Detection Accuracy Under Variable Illumination
- Authors: H. Kim et al.
- Venue: Safety Science (2023)
- DOI: 10.1016/j.ssci.2023.106123
- Decision: `ALREADY_IMPLEMENTED`

## Multi-modal Sensor Fusion for Robust Driver Behavior Monitoring
- Authors: Z. Liu et al.
- Venue: IEEE Transactions on Intelligent Vehicles (2024)
- DOI: 10.1109/TIV.2024.3315678
- Decision: `NOT_RELEVANT`
