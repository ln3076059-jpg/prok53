...
param(
    ...
    [ValidateSet("phone_detector", "seatbelt_detector", "seatbelt_classifier")]
    [string[]]$Only = @(),
...
if (-not $Working) {
    $Working = Join-Path $bundleRoot "runs\v2_pretrain_pending_approval"
}
...
try {
    if ($InstallDependencies) {
        Invoke-CheckedPython -Arguments @(
            "-m", "pip", "install", "-r", "requirements-windows-cu128.txt"
        )
        Invoke-CheckedPython -Arguments @(
            "-m", "pip", "install", "-r", "requirements-kaggle.txt"
        )
    }

    $baseArguments = @(
        "-m", "training.v2_pretrain_portable_runner",
        "--bundle", $bundleRoot,
        "--profile", $Profile
    )
    if ($BackupDirectory) {
    ...
    if ($Mode -eq "train" -and -not $SkipSmoke) {
        $smokeArguments = $baseArguments + @("--working", "$Working-smoke", "--smoke")
        if ($Only.Count -gt 0) {
            $smokeArguments += "--only"
            $smokeArguments += $Only
        }
